...
```
  ...
  │  │ (nginx-ingress-controller, traefik 등)           │   │
  │  │                                                  │   │
  ...
  │ • TLS 설정, 어노테이션(비표준 기능) 등                    │
  │ ※ 인프라 설정 + 라우팅 규칙이 하나에 혼재                 │
  └──────────────────────────┬──────────────────────────────┘
                             │
                    ┌────────▼────────┐
                    │ Backend Service │
...
```
  ...
  │                                                         │
  │  • 리스너(포트, 프로토콜, TLS) 정의                      │
  │  • 컨트롤러가 자동으로 LB Service + Envoy Pod 생성       │
  │  • EXTERNAL-IP: 172.16.200.1                            │
...
```
  ...
  │ Gateway                                           │
  │                                                   │
  │  X-Version: v2   ──────────────► app-v2 (새 버전) │
  │  X-Debug: true   ──────────────► app-debug        │
  │  (기본)           ──────────────► app-v1 (안정 버전)│
  └──────────────────────────────────────────────────┘
```
