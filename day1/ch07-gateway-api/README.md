...
```mermaid
graph TD
    IC["<b>IngressClass</b><br/>어떤 Ingress Controller를 사용할지 정의"]
    IC -- "참조" --> Controller

    subgraph Controller["Ingress Controller (Deployment + Service)"]
        Pods["<b>Ingress Controller Pod(s)</b><br/>(nginx-ingress-controller, traefik 등)<br/>• Ingress 리소스를 감시(watch)<br/>• 자체 설정을 동적으로 갱신<br/>• 트래픽을 직접 프록시"]
        LBSvc["<b>LoadBalancer Service</b><br/>컨트롤러가 직접 생성/관리하는 LB<br/>EXTERNAL-IP: 172.16.200.x"]
    end

    Controller -- "라우팅 규칙 참조" --> Ingress["<b>Ingress</b><br/>• host: myapp.example.com<br/>• path: /api → backend-service:80<br/>• TLS 설정, 어노테이션(비표준 기능) 등<br/><i>※ 인프라 설정 + 라우팅 규칙이 하나에 혼재</i>"]
    Ingress --> Backend1["Backend Service (Pod들)"]
```
...
```mermaid
graph TD
    GC["<b>GatewayClass</b><br/>어떤 Gateway 구현체(컨트롤러)를 사용할지 정의"]
    GC -- "참조" --> GW["<b>Gateway</b> (= LB 진입점 자체)<br/>• 리스너(포트, 프로토콜, TLS) 정의<br/>• 컨트롤러가 자동으로 LB Service + Envoy Pod 생성<br/>• EXTERNAL-IP: 172.16.200.1<br/>• 어떤 네임스페이스의 Route를 허용할지 설정<br/><i>※ 인프라 관리자가 관리</i>"]
    GW -- "parentRefs로 연결" --> HR["<b>HTTPRoute</b> (앱 개발자가 관리)<br/>• hostnames: myapp.example.com<br/>• 경로, 헤더, 가중치 기반 라우팅 규칙<br/>• backendRefs로 대상 Service 지정<br/><i>※ 라우팅 규칙만 담당 (인프라 설정과 분리)</i>"]
    HR --> Backend2["Backend Service (Pod들)"]
```
...
graph TD
    subgraph infra["인프라 관리자 영역"]
        gwclass["<b>GatewayClass</b><br/>• 어떤 구현체를 사용할지 정의<br/>• 예: Cilium, Istio, Envoy Gateway 등"]
        gateway["<b>Gateway</b><br/>• 리스너(포트, 프로토콜, 호스트) 정의<br/>• 실제 로드밸런서 인스턴스에 대응<br/>• 어떤 네임스페이스의 Route를 허용할지 설정"]
        gwclass --> gateway
    end
    subgraph dev["앱 개발자 영역"]
        httproute["<b>HTTPRoute</b><br/>• 호스트 기반 라우팅<br/>• 경로(path) 기반 라우팅<br/>• 헤더 기반 라우팅<br/>• 백엔드 Service 연결"]
    end
    gateway --> httproute
```
...
```mermaid
graph TD
    S1["1. 인프라 관리자: GatewayClass 생성<br/>(Cilium 컨트롤러 지정)"]
    S2["2. 인프라 관리자: Gateway 생성<br/>(리스너, TLS 등 설정)"]
    S2a["Cilium이 자동으로<br/>LoadBalancer Service 생성"]
    S2b["LB-IPAM이 External IP 할당<br/>(172.16.200.1)"]
    S2c["BGP로 외부 라우터에 경로 광고"]
    S3["3. 앱 개발자: HTTPRoute 생성<br/>parentRefs → Gateway 연결<br/>hostnames → 호스트 매칭<br/>backendRefs → 대상 Service"]
    S4["4. 외부 클라이언트 접속<br/>echo.basphere.dev"]
    S4a["DNS → 172.16.200.1<br/>→ BGP 경로로 클러스터 도달"]
    S4b["Gateway가 Host 헤더 확인<br/>→ echo-route 매칭"]
    S4c["echo-server Service → Pod"]
