...
```mermaid
graph LR
    browser["브라우저"] --> nodeport["모든 노드:3xxxx<br/>(wrk-0, wrk-1, wrk-2~5)"]
    subgraph cluster["클러스터 내부"]
...
spec:
  ...
  ports:
    - protocol: TCP
      ...
      targetPort: 80      # Pod의 컨테이너 포트
      # nodePort를 생략하면 K8s가 30000-32767 범위에서 자동 할당합니다
      # (여러 수강생이 동시에 생성해도 충돌하지 않음)
```
...
# 2. Service 확인 — K8s가 자동 할당한 NodePort를 확인합니다
kubectl get svc nginx-nodeport

# 예상 출력 (포트 번호는 자동 할당되므로 다를 수 있음):
# NAME             TYPE       CLUSTER-IP     EXTERNAL-IP   PORT(S)        AGE
# nginx-nodeport   NodePort   10.96.xx.xx    <none>        80:3xxxx/TCP   10s

# 3. 할당된 NodePort 번호 확인
...
# 5. 클러스터 내 Pod에서 NodePort로 접근 테스트
NODE_IP=$(kubectl get nodes -o jsonpath='{.items[3].status.addresses[0].address}')
...
> **핵심**: NodePort는 **모든 노드의 동일한 포트**로 접근할 수 있습니다. 어떤 노드에 요청하든 Service가 알맞은 Pod로 트래픽을 전달합니다. `nodePort`를 생략하면 K8s가 자동으로 포트를 할당하므로, 여러 수강생이 동시에 생성해도 포트 충돌이 발생하지 않습니다.
