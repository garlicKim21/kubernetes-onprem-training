...
graph TD
    subgraph userspace["사용자 공간"]
        agent["<b>Cilium Agent (cilium-agent)</b><br/>• eBPF 프로그램 로드/관리<br/>• 정책 관리"]
    end
    subgraph kernelspace["커널 공간"]
        ebpf["<b>eBPF 프로그램</b><br/>• 패킷 필터링/라우팅<br/>• 서비스 로드밸런싱<br/>• 네트워크 정책 적용<br/>• 네트워크 관측 (Hubble)"]
        netstack["Linux 네트워크 스택"]
    end
    agent --> ebpf
    ebpf --> netstack
```
...
```mermaid
graph TD
    A["1. LoadBalancer Service 생성"] --> B["2. Cilium LB-IPAM이 IP Pool에서 IP 할당<br/>(172.16.200.0/24 대역에서 선택)"]
    B --> C["3. Service의 EXTERNAL-IP에 할당된 IP 설정"]
    C --> D["4. BGP로 외부 라우터에 경로 광고<br/>172.16.200.x는 이 클러스터로 오세요"]
    D --> E["5. 외부 트래픽이 LB IP로 도달 → Pod에 전달"]
```
...
```mermaid
graph TD
    router["<b>OPNsense Router</b><br/>ASN 65000 · 10.254.0.1<br/>172.16.200.0/24로의 트래픽은<br/>Kubernetes 클러스터(ASN 65100)로"]
    router -- "BGP Session" --> wrk0["<b>wrk-0</b><br/>ASN 65100<br/>Cilium BGP Agent"]
    router -- "BGP Session" --> wrk1["<b>wrk-1</b><br/>ASN 65100<br/>Cilium BGP Agent"]
    router -- "BGP Session" --> wrk2["<b>wrk-2</b><br/>ASN 65100<br/>Cilium BGP Agent"]
    router -- "BGP Session" --> wrk3["<b>wrk-3</b><br/>ASN 65100<br/>Cilium BGP Agent"]
    router -- "BGP Session" --> wrk4["<b>wrk-4</b><br/>ASN 65100<br/>Cilium BGP Agent"]
    router -- "BGP Session" --> wrk5["<b>wrk-5</b><br/>ASN 65100<br/>Cilium BGP Agent"]
```
...
```mermaid
graph TD
    ippool["<b>CiliumLoadBalancerIPPool</b><br/>(IP 풀 정의)<br/>cidr: 172.16.200.0/24"]
    clusterconfig["<b>CiliumBGPClusterConfig</b><br/>(BGP 피어 + 노드 선택)"]
    peerconfig["<b>CiliumBGPPeerConfig</b><br/>(AFI/SAFI + Graceful Restart)"]
    advert["<b>CiliumBGPAdvertisement</b><br/>(어떤 Service IP를 광고할지)"]
    svc["<b>Service</b>"]

    clusterconfig -- "peerConfigRef" --> peerconfig
    peerconfig -- "advertisements.matchLabels" --> advert
    ippool -- "IP 할당" --> svc
    advert -- "BGP 경로 광고" --> svc
```
...
```mermaid
graph TD
    A1["1. CiliumLoadBalancerIPPool 생성<br/>172.16.200.0/24 대역 등록"] --> A2["2. CiliumBGPClusterConfig 생성<br/>Worker 노드가 OPNsense와 BGP 세션 수립"]
    A2 --> A3["3. LoadBalancer Service 생성<br/>Cilium LB-IPAM이 172.16.200.x IP 할당"]
    A3 --> A4["4. CiliumBGPAdvertisement<br/>BGP로 172.16.200.x → 이 클러스터 경로 광고"]
    A4 --> A5["5. OPNsense 라우터가 경로를 학습"]
    A5 --> A6["6. 외부 클라이언트 → OPNsense → Worker 노드 → Pod"]
```
...
```mermaid
graph TD
    client["Client"] --> router["OPNsense (Router)<br/>BGP로 6개 경로 수신 → ECMP로 분산"]
    router --> wrk0["wrk-0<br/>BGP ✅<br/><i>Pod 없음!</i>"]
    router --> wrk1["wrk-1<br/>BGP ✅<br/><b>Pod-A</b>"]
    router --> wrk2["wrk-2<br/>BGP ✅<br/><i>Pod 없음!</i>"]
    router --> wrk3["wrk-3<br/>BGP ✅<br/><i>Pod 없음!</i>"]
    router --> wrk4["wrk-4<br/>BGP ✅<br/><b>Pod-B</b>"]
    router --> wrk5["wrk-5<br/>BGP ✅<br/><i>Pod 없음!</i>"]
    wrk0 -. "재전송 (hop 추가)" .-> wrk1
```

> 모든 노드가 광고: "나를 통해 172.16.200.x에 도달할 수 있다"

문제점:
- Pod가 없는 노드(wrk-0, wrk-2, wrk-3, wrk-5)로 갈 수 있음 → 내부에서 Pod 있는 노드로 재전송 (불필요한 hop)
- 재전송 시 SNAT 발생 → 클라이언트 원본 IP가 Pod에서 보이지 않음
...
```mermaid
graph TD
    client1["Client"] --> router1["OPNsense<br/>6개 경로 수신, ECMP"]
    router1 --> w0c["w-0 BGP ✅"]
    router1 --> w1c["w-1 BGP ✅<br/><b>PodA</b>"]
    router1 --> w2c["w-2 BGP ✅"]
    router1 --> w3c["w-3 BGP ✅"]
    router1 --> w4c["w-4 BGP ✅<br/><b>PodB</b>"]
    router1 --> w5c["w-5 BGP ✅"]
    w0c -. "hop 추가 + SNAT" .-> w1c
```

> Pod 없는 노드로도 트래픽이 갈 수 있음 → 다른 노드로 재전송

**Local 모드:**
```mermaid
graph TD
    client2["Client"] --> router2["OPNsense<br/>2개 경로만 수신, ECMP"]
    router2 --> w1l["w-1 BGP ✅<br/><b>PodA</b>"]
    router2 --> w4l["w-4 BGP ✅<br/><b>PodB</b>"]
```

> Pod 있는 노드만 광고 → 직접 도달, SNAT 없음
...
| 불필요한 hop | 발생 가능 | 없음 |
| 클라이언트 IP 보존 | ❌ (SNAT) | ✅ |
| 부하 분산 균등성 | 높음 (모든 노드 경유) | ⚠️ 불균등 가능 (아래 참고) |
| 사용 시나리오 | 일반적인 서비스 | 클라이언트 IP가 필요한 경우 (로깅, 인증) |

#### Local 모드의 부하 분산 불균등 문제

Local 모드에서는 BGP ECMP가 **노드 단위**로 균등 분배하므로, 한 노드에 Pod가 여러 개 있으면 부하가 불균등해질 수 있습니다.

```mermaid
graph TD
    client3["Client<br/>100 requests"] --> router3["OPNsense<br/>ECMP: 50% / 50% (노드 단위 균등)"]
    router3 -- "50 req" --> wrk1n["wrk-1"]
    router3 -- "50 req" --> wrk4n["wrk-4"]
    wrk1n -- "25 req" --> podA["Pod-A"]
    wrk1n -- "25 req" --> podBx["Pod-B"]
    wrk4n -- "50 req" --> podC["Pod-C<br/>혼자 50개 처리!"]
```

> 결과: Pod-A=25req, Pod-B=25req, Pod-C=50req → Pod-C가 2배의 부하를 받음 (불균등!)

비교 -- Cluster 모드에서 같은 상황:

> 어떤 노드로 가든 Cilium이 3개 Pod에 균등 분배
> 결과: Pod-A 약 33req, Pod-B 약 33req, Pod-C 약 33req → 모든 Pod가 균등한 부하 (하지만 SNAT 발생)

> **정리**: Local 모드는 클라이언트 IP 보존이 필요할 때 사용하되,
> Pod 배치가 노드별로 균등하도록 `topologySpreadConstraints`를 함께 설정하는 것이 좋습니다.
