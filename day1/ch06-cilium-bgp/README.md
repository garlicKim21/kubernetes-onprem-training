...
- **Cilium Agent (cilium-agent)**: eBPF 프로그램 로드/관리, 정책 관리
- **eBPF 프로그램**: 패킷 필터링/라우팅, 서비스 로드밸런싱, 네트워크 정책 적용, 네트워크 관측 (Hubble)
...
```mermaid
graph TD
    A["1. LB Service 생성"] --> B["2. LB-IPAM이 IP 할당"]
    B --> C["3. EXTERNAL-IP 설정"]
    C --> D["4. BGP 경로 광고"]
    D --> E["5. 외부 트래픽 → Pod 전달"]
```
...
1. LoadBalancer 타입 Service가 생성됨
2. Cilium LB-IPAM이 IP Pool(172.16.200.0/24)에서 IP를 할당
3. Service의 EXTERNAL-IP에 할당된 IP를 설정
4. BGP로 외부 라우터에 경로 광고 ("172.16.200.x는 이 클러스터로")
5. 외부 트래픽이 LB IP로 도달하여 Pod에 전달
...
```mermaid
graph TD
    router["OPNsense Router<br/>ASN 65000"]
    router -- "BGP" --> wrk0["wrk-0 (ASN 65100)"]
    router -- "BGP" --> wrk1["wrk-1 (ASN 65100)"]
    router -- "BGP" --> wrk2["wrk-2 (ASN 65100)"]
    router -- "BGP" --> wrk3["wrk-3 (ASN 65100)"]
    router -- "BGP" --> wrk4["wrk-4 (ASN 65100)"]
    router -- "BGP" --> wrk5["wrk-5 (ASN 65100)"]
```

**BGP 토폴로지 상세:**

- **OPNsense Router** (ASN 65000, 10.254.0.1): 172.16.200.0/24로의 트래픽을 Kubernetes 클러스터로 전달
- **Worker 노드** (ASN 65100): 각 노드에서 Cilium BGP Agent가 실행되어 라우터와 BGP 세션을 유지
...
- **CiliumLoadBalancerIPPool**: IP 풀 정의 (cidr: 172.16.200.0/24) → Service에 IP 할당
- **CiliumBGPClusterConfig**: BGP 피어 및 노드 선택 → peerConfigRef로 PeerConfig 참조
- **CiliumBGPPeerConfig**: AFI/SAFI 및 Graceful Restart 설정 → advertisements.matchLabels로 Advertisement 참조
- **CiliumBGPAdvertisement**: 어떤 Service IP를 BGP로 광고할지 정의
...
```mermaid
graph TD
    A1["1. IP Pool 생성"] --> A2["2. BGP ClusterConfig 생성"]
    A2 --> A3["3. LB Service 생성"]
    A3 --> A4["4. BGP 경로 광고"]
    A4 --> A5["5. 라우터가 경로 학습"]
    A5 --> A6["6. Client → Router → Node → Pod"]
```
...
5. **OPNsense 라우터**가 경로를 학습
6. 외부 클라이언트 → OPNsense → Worker 노드 → Pod
...
```mermaid
graph TD
    client["Client"] --> router["OPNsense<br/>6경로 ECMP"]
    router --> wrk0["wrk-0 ✅ (no Pod)"]
    router --> wrk1["wrk-1 ✅ Pod-A"]
    router --> wrk2["wrk-2 ✅ (no Pod)"]
    router --> wrk3["wrk-3 ✅ (no Pod)"]
    router --> wrk4["wrk-4 ✅ Pod-B"]
    router --> wrk5["wrk-5 ✅ (no Pod)"]
    wrk0 -. "재전송 + SNAT" .-> wrk1
```
