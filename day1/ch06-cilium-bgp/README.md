...
모든 노드가 광고: "나를 통해 172.16.200.x에 도달할 수 있다"
  ...
  문제점:
  - Pod가 없는 노드(wrk-0, wrk-2, wrk-3, wrk-5)로 갈 수 있음
    → 내부에서 Pod 있는 노드로 재전송 (불필요한 hop)
...
```
예시: Pod 3개가 wrk-1에 2개, wrk-4에 1개 배치된 경우
   ...
   │ 50 req│         │ 50 req│
   ├───────┤         ├───────┤
   │ Pod-A │ 25 req  │ Pod-C │ 50 req  ← 혼자 50개 처리!
   │ Pod-B │ 25 req  │       │
   └───────┘         └───────┘

결과: Pod-A=25req, Pod-B=25req, Pod-C=50req
  → Pod-C가 2배의 부하를 받음 (불균등!)
```
...
결과: Pod-A≈33req, Pod-B≈33req, Pod-C≈33req
  → 모든 Pod가 균등한 부하 (하지만 SNAT 발생)
```
