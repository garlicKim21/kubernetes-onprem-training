...
graph TD
    subgraph prom["Prometheus Server"]
        scraper["Scraper (수집기)"]
        tsdb["TSDB (시계열DB)"]
        promql["PromQL Engine (쿼리 엔진)"]
    end

    scraper -- "HTTP GET /metrics<br/>(주기적 스크래핑)" --> app1["앱1<br/>/metrics"]
    scraper --> app2["앱2<br/>/metrics"]
    scraper --> app3["앱3<br/>/metrics"]
    promql -- "쿼리" --> grafana["Grafana<br/>(시각화)"]
    promql -- "쿼리" --> alertmgr["Alertmanager<br/>(알림 전송)"]
```
...
graph TD
    subgraph stack["kube-prometheus-stack"]
        prometheus["<b>Prometheus Server</b>"]
        grafana["<b>Grafana</b>"]
        alertmanager["<b>Alertmanager</b>"]
        nodeexp["node-exporter"]
        ksm["kube-state-metrics"]
        operator["Prometheus Operator (CRD)"]

        prometheus -- "메트릭 수집" --> nodeexp
        prometheus -- "메트릭 수집" --> ksm
        prometheus -- "시각화" --> grafana
        operator -- "설정 관리" --> prometheus
    end
```
...
2. 로그인 정보:
   - **수강생용**: 사용자명 `student`, 비밀번호 `<강사에게 문의>` (읽기 전용)
   - **관리자용**: 사용자명 `admin`, 비밀번호 `<강사에게 문의>` (전체 권한)
...
```mermaid
graph LR
    prom2["<b>Prometheus</b><br/>Alert Rules"] -- "Alert" --> am["<b>Alertmanager</b><br/>• 그룹핑<br/>• 중복 제거<br/>• 라우팅<br/>• 묵음(Silence)"]
    am -- "알림 전송" --> targets["Slack / Email<br/>PagerDuty / Webhook"]
```
